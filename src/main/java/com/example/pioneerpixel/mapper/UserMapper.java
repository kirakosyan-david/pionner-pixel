...
public interface UserMapper {
  ...
  @Mappings({
    ...
    @Mapping(
        target = "email",
        expression =
            "java(user.getEmails().isEmpty() ? null : user.getEmails().get(0).getEmail())"),
    @Mapping(
        target = "phone",
        expression = "java(user.getPhones().isEmpty() ? null : user.getPhones().get(0).getPhone())")
  })
  ...
  @Mapping(
      target = "email",
      expression =
          "java(user.getEmails().isEmpty() ? null : user.getEmails().iterator().next().getEmail())")
  @Mapping(
      target = "phone",
      expression =
          "java(user.getPhones().isEmpty() ? null : user.getPhones().iterator().next().getPhone())")
