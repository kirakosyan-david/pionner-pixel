...
public class EmailDataServiceImpl implements EmailDataService {
  ...
  public UserEmailDtoResponse addEmailOrUpdate(Long userId, UserEmailDtoRequest email) {
    ...
    User user =
        userRepository
            .findById(userId)
            .orElseThrow(
                () -> new PhoneAndEmailOperationException("User not found with id: " + userId));
    ...
    EmailData emailData =
        emailDataRepository
            ...
            .orElseGet(() -> EmailData.builder().user(user).email(email.getEmail()).build());
  ...
  public void deleteEmail(Long userId, UserEmailDtoRequest email) {
    ...
    EmailData emailData =
        emailDataRepository
            .findByEmailAndUserId(email.getEmail(), userId)
